---
source_type: 'developers'
source_area: 'codex_track'
source_url: 'https://developers.openai.com/codex/tracks/native-development'
source_last_modified: '2026-04-25T12:19:47Z'
source_etag: '"a2af58e9b50bb66c0bd91aeb35cc5f39"'
codex_cli_versions: ["0.125.0", "0.128.0", "0.129.0", "0.130.0"]
codex_cli_versions_raw: ["codex-cli 0.125.0", "codex-cli 0.128.0", "codex-cli 0.129.0", "codex-cli 0.130.0"]
---

# Redirecting to: /codex/use-cases/collections/native-development

Source: https://developers.openai.com/codex/tracks/native-development

[Redirecting from `/codex/tracks/native-development/` to `/codex/use-cases/collections/native-development`](/codex/use-cases/collections/native-development)

